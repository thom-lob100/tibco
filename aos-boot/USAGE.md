...
코드는 회사 표준 `tibrvnative.jar` API와 동일한 시그니처로 작성되어 있다.
바꿔야 하는 것은 환경/설정이다.

1. **tibrvnative 설치 (필수)** — 로컬 Maven 저장소의 스텁을 회사의 진짜 jar로 덮어쓴다:
   ```
   ...
   `real` profile은 `aos.rendezvous.require-native=true`라 스텁 기동을 즉시 거부한다.
   스텁으로 기동하면 `[tibrvnative-stub]` 경고가 출력된다 — **운영 로그에 이 문구가 보이면
   잘못 배포된 것.**
...
3. **DB 교체 (persistent 큐 사용 시)** — `spring.datasource`를 실제 DB로 변경.
   ...
   `GENERATED BY DEFAULT AS IDENTITY`, 샘플의 `MERGE INTO ... KEY`.
    Oracle 12c+에서는 폴러의 `FETCH FIRST ... ROWS ONLY`를 지원해야 한다. DBA 제출용
    컬럼 명세와 Oracle 타입 변환 규칙은 **`TABLES.md`** 참조.
...
aos:
  rendezvous:
    ...
    persistent-queue:              # persistent command DB 재처리 큐 (listener별로 격리)
      enabled: true
      poll-interval: 5.0
...
- persistent 큐 모니터링:
  ...
  ```
- 같은 listener의 여러 인스턴스는 조건부 UPDATE로 행을 안전하게 claim한다. 다른 listener는
  `listener` 컬럼으로 격리되므로, 서로 다른 역할이 같은 행을 재처리하지 않는다.
